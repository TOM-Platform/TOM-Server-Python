...
class VideoOutput(BaseComponent):
    ...
    def play(self, raw_data):
        ...
        if origin == base_keys.YOLOV8_PROCESSOR:  # or origin == base_keys.CAMERA_WIDGET:
            frame = raw_data[base_keys.YOLOV8_FRAME]  # or raw_data[base_keys.CAMERA_FRAME]
    ...
    def _handle_video_data(self, frame, camera_frame_width, camera_frame_height, camera_fps):
        # _logger.debug(f"VideoOutput::_handle_video_data() - camera_frame_width: [{str(camera_frame_width)}], camera_frame_height: [{str(camera_frame_height)}], camera_fps: [{str(camera_fps)}]")

        if self.save:
            if self.video_writer is None:
                self.video_writer = cv2.VideoWriter(self.save_path, cv2.VideoWriter_fourcc(*'XVID'),
                                                    camera_fps,
