...
class InvalidDataOriginException(LearningException):
    ...
    def __str__(self):
        return f"{self.__class__.__name__} - Data received from an \
            invalid source: {self.origin}"
...
class ErrorDetectingTextException(LearningException):
    ...
    def __str__(self):
        return f"{self.__class__.__name__} - Detecting Text \
            failed with error: {self.error_msg}"
...
class ErrorGenerateTextException(LearningException):
    ...
    def __str__(self):
        return f"{self.__class__.__name__} - Generating Text \
            failed with error: {self.error_msg}"
